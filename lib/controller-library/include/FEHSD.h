...
public:
     ...
     *
     * @param fptr File pointer to be moved
     * @param offset number of bytes to offset from position
     * @param position position from where offset is added. Values include SEEK_SET, SEEK_CUR, and SEEK_END
     * @return int zero if success, nonzero otherwise
     */

    int FSeek(FEHFile *fptr, long int offset, int position = SEEK_SET);

    /**
     * @brief Prints the contents of the file from SD card to Serial Monitor
     *
     * @param str Name of the file to be opened
     */

    void FlushToConsole(const char *str);

    /**
     * @brief Prints the contents of the file from SD card to Serial Monitor
     *
     * @param fptr FEHFile pointer already opened for reading
     */
    void FlushToConsole(FEHFile *fptr);

    FEHSD();

private:
    FEHFile *files[25];
    int my_vsscanf(const char *str, const char *format, va_list args);
    int numberOfFiles = 0;
};
